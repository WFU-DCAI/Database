...
# tree_issues
tree_issues_df = (
    df[['root_stone', 'root_grate', 'root_other', 'trunk_wire',
...
for col in tree_issues_df.columns:
    tree_issues_df[col] = tree_issues_df[col].map({'Yes': 1, 'No': 0})

tree_issues_df = empty_to_null(tree_issues_df)

tree_issues_df.to_sql(
    'tree_issues',
    conn,
    if_exists='append',
    index=False
